...
| **Opt #3** | Opt | Human-in-the-Loop Daten-Locking | `backend/db/schema.py`, routes | ✅ Erledigt |
| **Opt #4** | Opt/UI | Feingranulare Settings-UI | `frontend/src/pages/Settings.tsx` | ✅ Erledigt |
| **Opt #5** | Opt | Modell-Kaskade (LLM Cascade Scenarios) | `backend/llm/classify.py`, prompts | ✅ Erledigt |
| **Opt #6** | Opt/UI | Schnellfilter-Verschiebung | `frontend/src/pages/Documents.tsx` | ✅ Erledigt |
...
*   **Problem:** `content_hashes` and `load_hashes()` sind ungenutzter toter Code in `storage.py`.
...
### ✅ Optimierung #3: Human-in-the-Loop Daten-Locking
*   **Problem:** Schutz manuell verifizierter Dokumente vor KI-Klassifizierungsdrifts oder automatischen Massen-Reklassifizierungen.
*   **Lösung:** Einführung der SQLite-Spalte `verified` in der Haupttabelle `documents`. Verifizierte Belege (`verified = 1`) werden für jegliche Inferenzupdates blockiert.
*   **Ergebnis:** Erfolgreich implementiert, voll verifiziert über `tests/test_documents_api.py`.

### ✅ Optimierung #4: Feingranulare Settings-UI
*   **Problem:** Kategorien-Mappings, Root-Ablagepfade, MFH-Wohneinheiten und Jahresordner-Nutzung ließen sich im UI bisher nicht granular bearbeiten.
*   **Lösung:** Einbau eines eleganten Inline-Kategorie-Editors in `Settings.tsx` mitsamt Stift-Symbol (`Pencil`), um alle Konfigurationen unkompliziert direkt im UI editieren zu können.
*   **Ergebnis:** Erfolgreich implementiert und im UI verifiziert.

### ✅ Optimierung #5: Modell-Kaskade (LLM Cascade Scenarios)
*   **Problem:** Klassifizierung über einen riesigen Monolith-Prompt kostet Inferenzzeit und birgt hohes Fehlerrisiko für lokale Modelle.
*   **Lösung:** Aufbrechen in eine 2-stufige Inferenz-Kaskade. Stufe 1 extrahiert nur Basis-Metadaten (Sender, Datum, Typ), Stufe 2 nutzt darauf aufbauend spezialisierte Folgeprompts.
*   **Ergebnis:** Vollständig integriert und über `tests/test_llm_classify.py` sowie `tests/test_llm.py` verifiziert.

### ✅ Optimierung #6: Schnellfilter-Verschiebung
*   **Problem:** Filter wie "Ausstehend" blockierten wertvollen Platz im globalen linken Navigationsmenü.
*   **Lösung:** Integration eines schnellen Filter-Buttons direkt im Tabellen-Suchkopf von `Documents.tsx`.
*   **Ergebnis:** Erfolgreich implementiert und im Frontend lückenlos verifiziert.
*   **Ergebnis:** Verifiziert.
